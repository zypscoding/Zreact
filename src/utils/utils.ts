...
export const disassembleUrl=(url)=>{
    let result:any = {},parm={};
    var urlParm="";
    if(url) {
        var url=url.split("?");
...
export const replaceLongData = (resp) => {
    resp = resp.replace(/"\w+":\s*\d{16,}/g, function(longVal){
...
export const fileDownLoad = async(url, role?: any) => {
    // let response:any = await api.postFileParamsForJson(url, role);
